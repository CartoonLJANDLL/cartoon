    ...
    <ul class="layui-tab-title">
    	<c:choose>
			<c:when test="${status==0 }">
        		...
        		<li><a href="/guomanwang/luntan_hot" id="selectbythreadnum">按热度排行</a></li>
			</c:when>
			<c:otherwise> 
				<li><a href="/guomanwang/luntan">按默认排名</a></li>
        		<li class="layui-this"><a href="/guomanwang/luntan_hot" id="selectbythreadnum">按热度排行</a></li>
			</c:otherwise>
		</c:choose> 
    </ul>
  </div>
    ...
    <c:choose>
		<c:when test="${status==0 }">
			 <c:if test="${page.pageNum!=1}">
		    	<a href="/guomanwang/luntan?pn=${page.pageNum-1 }">上一页</a>
		     </c:if>
		     <c:forEach items="${page.navigatepageNums }" var="page_num" varStatus="status">
		        <a href="/guomanwang/luntan?pn=${ page_num}" class='<c:if test="${page_num==page.pageNum }">laypage-curr</c:if>'>${ page_num}</a>
		     </c:forEach>
			 <c:if test="${page.pageNum!=page.pages}">
		    	<a href="/guomanwang/luntan?pn=${page.pageNum+1 }">下一页</a>
		     </c:if> 
		</c:when>
		<c:otherwise> 
			 <c:if test="${page.pageNum!=1}">
		    	<a href="/guomanwang/luntan_hot?pn=${page.pageNum-1 }">上一页</a>
		     </c:if>
		     <c:forEach items="${page.navigatepageNums }" var="page_num" varStatus="status">
		        <a href="/guomanwang/luntan_hot?pn=${ page_num}" class='<c:if test="${page_num==page.pageNum }">laypage-curr</c:if>'>${ page_num}</a>
		     </c:forEach>
			 <c:if test="${page.pageNum!=page.pages}">
		    	<a href="/guomanwang/luntan_hot?pn=${page.pageNum+1 }">下一页</a>
		     </c:if> 
		</c:otherwise>
	</c:choose>
    </div>
