...
def getContent(url):
    ...
    contents = bs.select('.ptxt > .tit > a')
    return contents

#获得资讯发布时间
def getTime(url):
    bs=getHtml (url)
    times = bs.select('.ptxt > .con > .tem > .time')
    return times
...
def insertNews(url):
    ...
    for content,time in zip(contentlist,timelist):
        ...
        newsObjs = [content.text, '默认',time.text,'动漫星空',content.get('href')]
...
i=1
while i<7:
    url= 'http://tag.gamersky.com/acg/news/9005-'+str(i)+'.html'
    insertNews(url)
    i=i+1